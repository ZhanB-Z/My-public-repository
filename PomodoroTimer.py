...
import time
import threading  #threading allows for execution of two things at the same time
#thanks to threading, we also can move the window, and the timer will continue.
class PomodoroTimer:

    def __init__(self):
        self.root = tk.Tk()
        self.root.geometry("600x300") #setting size of the window
        self.root.title("Pomodoro Timer")
        ###self.root.tk.call => to use an image for icon
        ...
        self.long_break_timer_label = ttk.Label(self.tab3, text="25:00", font=("Ubuntu", 48))
        ...
        self.reset = ttk.Button(self.grid_layout, text="Reset", command=self.reset_clock)
        ...
        self.root.mainloop() #if this method is not used, there will be no window. this is an event-looping method

    def start_timer_thread(self):
        if not self.running:
            t = threading.Thread(target = self.start_timer) #again, threading allows for two things to run simultaneously
            t.start()
            self.running = True
    ...
    def start_timer(self):
        ...
        if timer_id == 1:
            ...
            while full_seconds > 0 and not self.stopped:
                ...
                self.root.update()
                time.sleep(1) #suspends execution oof the current thread for a given number of seconds
                full_seconds -= 1
